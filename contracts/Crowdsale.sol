...
contract Crowdsale is Owner{
    ...
    **/
     ...
     */
    function ownerCheckGoalIfReached(bool isSuccess) afterDeadline onlyOwner public{
        if (isSuccess) {
     ...
     * 失败，执行退款eth
     */
    function refundEth(address buyerAddress) afterDeadline public{
     ...
     * 众筹人自己退币
     */
    function buyerSelfRefundEth() afterDeadline public{
        require(ownerCheckGoalReached && !fundingGoalReached); //需要执行完 checkGoalReached()／ico失败
     ...
     *  发币
     */
    function sendTokenToBuyer(address buyerAddress) afterDeadline public{
     ...
     * 完成融资目标时,项目方收款
     */
    function beneficiaryWithdrawal() afterDeadline public{
     ...
     * 或 owner发 eth to 项目方 
     */
    function ownerDealWithEth(address toAddress) afterDeadline onlyOwner public{
     ...
     */
    function ownerDealWithEthCoins(address toAddress,uint256 amount) afterDeadline onlyOwner public{
        require(ownerCheckGoalReached && fundingGoalReached); //需要执行完 checkGoalReached(),并且//ico成功
     ...
     * token转移 onlyOwner
     */
    function ownerTransferToken(address toAddress,uint256 amount) afterDeadline onlyOwner public{
